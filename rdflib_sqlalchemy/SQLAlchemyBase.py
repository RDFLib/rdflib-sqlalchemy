...
Index('idx_ns_prefix', Namespaces.ns, Namespaces.prefix)
Index('idx_subject_predicate', Triples.subject, Triples.predicate)
Index('idx_predicate_object', Triples.predicate, Triples.object)
Index('idx_subject_object_context', Triples.subject, Triples.object,
                                    Triples.context)
Index('idx_predicate_object_context', Triples.predicate, Triples.object,
                                        Triples.context)
Index('idx_subject_predicate_context', Triples.subject, Triples.predicate,
                                        Triples.context)
Index('idx_subject_predicate_object_context', Triples.subject, Triples.predicate,
                                            Triples.object,Triples.context)
...
class SQLAlchemy(Store):
    ...
    def __init__(self, identifier=None, configuration=None):
        ...
        super(SQLAlchemy, self).__init__(identifier=identifier, configuration=configuration)
    ...
    def triples(self, triple_pattern, context=None):
        ...
        return self.dbsession.query(Triples).filter_by(
    ...
    def __len__(self, context=None):
        ...
        if context is not None:
            return self.dbsession.query(Triples).filter(quoted!=True).all()
        else:
            return self.dbsession.query(Triples).filter(context=context).all()
    ...
    def contexts(self, triple=None):
        ...
        if triple is not None:
            ...
            return self.dbsession.query(Triples).select(context).distinct().all()
    ...
    def prefix(self, namespace):
        """ """
        return self.session.query(Namespaces).filter(
                namespace = namespace
                ).one().prefix

    def namespace(self, prefix):
        """ """
        return self.dbsession.query(Namespaces).filter(
                prefix = prefix
                ).one().namespace
